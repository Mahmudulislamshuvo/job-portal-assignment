const BenefitsEmployee = () => {
  return (
    <>
      <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="flex items-start gap-3 p-4 rounded-lg bg-muted/50">
          <div class="flex-shrink-0 w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
            <i data-lucide="users" class="h-5 w-5 text-primary"></i>
          </div>
          <div>
            <h3 class="font-semibold text-sm mb-1">Access Top Talent</h3>
            <p class="text-xs text-muted-foreground">
              Connect with thousands of qualified candidates actively looking
              for opportunities
            </p>
          </div>
        </div>
        <div class="flex items-start gap-3 p-4 rounded-lg bg-muted/50">
          <div class="flex-shrink-0 w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
            <i data-lucide="zap" class="h-5 w-5 text-primary"></i>
          </div>
          <div>
            <h3 class="font-semibold text-sm mb-1">Easy Job Posting</h3>
            <p class="text-xs text-muted-foreground">
              Post jobs in minutes with our intuitive interface and smart
              templates
            </p>
          </div>
        </div>
        <div class="flex items-start gap-3 p-4 rounded-lg bg-muted/50">
          <div class="flex-shrink-0 w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
            <i data-lucide="chart-line" class="h-5 w-5 text-primary"></i>
          </div>
          <div>
            <h3 class="font-semibold text-sm mb-1">Smart Analytics</h3>
            <p class="text-xs text-muted-foreground">
              Track applications and optimize your hiring with detailed insights
            </p>
          </div>
        </div>
      </div>
    </>
  );
};

export default BenefitsEmployee;
